...
var SOCKET_TIMEOUT = process.env.SOCKET_TIMEOUT || 3000;
var SERVER_TIMEOUT = process.env.SERVER_TIMEOUT || 7000;
...
var server = http.createServer(function(req, res) {
  ...
  if (toobusy()) return onerror(req, res)(new Error('server too busy'));
...
function chooseErrorHandler(handler) {
  ...
  return function(req, res, reqd) {
    ...
    return function(err) {
      ...
      // If we aren't in the domain don't shut down our worker
      if (!reqd) return;

      // If we're not in cluster mode don't shut down
